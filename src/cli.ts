...
async function parseRequest(args: readonly string[]): Promise<ParsedRequest> {
  ...
  let bodySource: "argument" | "file" | "stdin" | undefined;
  const query: string[] = [];
  ...
  for (let index = 2; index < args.length; index += 1) {
    ...
    if (option === "--body") {
      if (bodySource) throw new CliError("Use only one request body source");
...
async function main(): Promise<void> {
  ...
  const apiUrl = process.env.PLUNK_API_URL;
  if (!apiUrl) throw new CliError("Missing PLUNK_API_URL");
